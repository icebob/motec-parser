...
class LDXParser {
  ...
  getChannelData(lap, channelName) {
    ...
    const startBeacon = this.data.beacons[lap - 2] ?? 0;
    const endBeacon = this.data.beacons[lap - 1] ?? null;

    const channelMs = 1000 / channel.rec_freq;
    const startNum = Math.round(startBeacon * 1000 / channelMs);
    const endNum = Math.round(endBeacon != null ? endBeacon * 1000 / channelMs : channel.n_data);
    const data = channel.data.slice(startNum, endNum);
